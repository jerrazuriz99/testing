...
def isOperation(string):
    ...
    if string[0] == '(' and string[len(string)-1] == ')':
        ...
        # Verificamos que el operador sea uno que reconocemos y que solo exista el numero correcto de argumentos
...
def operation(string):
    tokens = splitArgs(string[1:len(string)-1])
    operator = tokens.pop(0)
    # Creamos el nodo correspondiente segun el operador y parseamos los demas argumentos 
    if operator == "+":
